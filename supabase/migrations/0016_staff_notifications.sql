-- Staff notifications for the Admin Dashboard
--
-- Creates: public.staff_notifications + triggers for new orders and customer chat messages.
...
begin
  ...
  loop
    ...
    values (staff_id, 'new_order', 'New order', 'Order #' || short_id || ' was placed.', 'order', new.id);
  end loop;
...
begin
  ...
  loop
    insert into public.staff_notifications (recipient_id, type, title, body, entity_type, entity_id)
    values (staff_id, 'customer_message', 'New message', 'Customer sent a message on order #' || short_id || '.', 'order', order_id);
  end loop;
